...
//               <Text style={styles.confirmationTableHeaderText}>Doses/Vial</Text>
//             </View>
...
import { createCoverage } from '@/api/coverageApi';
...
function DailyCoverageScreen({ navigation }) {
  ...
  const confirmationVaccines = vaccineFields.filter(item => {
    return Number(coverage[item.field] || 0) > 0;
  });

  const buildVaccinesPayload = () => {
    return vaccineFields
      .filter(item => Number(coverage[item.field] || 0) > 0)
      .map(item => ({
        vaccine: item.label,
        doses: Number(coverage[item.field]),
      }));
  };

  const handleSave = () => {
    if (!hasAnyCoverage || saving) {
      return;
    }
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      const vaccines = buildVaccinesPayload();
      ...
      const payload = {
        coverageDate: formatDateForApi(coverageDate),
        vaccines,
      };

      console.log('Daily Coverage Payload:', payload);

      const response = await createCoverage(payload);

      console.log('Daily Coverage API Response:', response);
      ...
      setResultModal({
        ...
        message: response?.message || 'Daily coverage saved successfully.',
      });
    } catch (error) {
      // console.error('Daily Coverage Save Error:', error);

      const message =
        error?.response?.data?.message ||
        error?.message ||
        'Unable to save daily coverage. Please try again.';
      ...
      <KeyboardScreen contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity
            ...
            onPress={handleSave}
            disabled={!hasAnyCoverage || saving}
          >
            {saving ? (
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.confirmationCard}>
              <View style={styles.confirmationTableHeaderRow}>
                ...
                <Text style={styles.confirmationTableHeaderText}>Doses</Text>
              </View>
